...
function getRanges(from, to, limit) {
  const ranges = [];
  let newLimit = limit;
  while (to - from > newLimit * newLimit) {
    newLimit *= newLimit;
  }
  for (let i = from; i <= to; i += newLimit) {
    ranges.push({ from: i, to: Math.min(to, i + newLimit - 1) });
  }
...
export default function getCollectionEntries(
  ...
  if (type === 'Iterable') {
    ...
    limitedEntries = hasMore
      ? [...entries, ...getRanges(from + limit, from - 1 + 2 * limit, limit)]
      : entries;
  } else {
